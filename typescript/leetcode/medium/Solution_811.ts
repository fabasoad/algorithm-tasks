function subdomainVisits(cpdomains: string[]): string[] {
  ...
  for (const item of cpdomains) {
    ...
    const visited: number = +pair[0];
    let domain: string = pair[1];
    let ind = -1;
    if (map.has(domain)) {
      map.set(domain, (map.get(domain) || 0) + visited);
    } else {
      map.set(domain, visited);
    }
    while ((ind = domain.indexOf('.')) != -1) {
  ...
  for (const domain of map.keys()) {
    res.push(`${map.get(domain)} ${domain}`)
  }
  return res;
}
